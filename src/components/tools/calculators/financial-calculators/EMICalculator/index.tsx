...
type LoanType = 'home' | 'car' | 'personal' | 'education' | 'other';
...
const TENURE_UNITS: { value: TenureUnit; label: string }[] = [
...
export function EMICalculator() {
  ...
  const calculateEMI = useCallback(() => {
    ...
    const years = tenureUnit === 'months' ? tenureNum / 12 : tenureNum;
    const monthlyRate = rateNum / 100 / 12;
    const numberOfInstallments = years * 12;

    let emiValue: number;
    if (monthlyRate === 0) {
      emiValue = principalNum / numberOfInstallments;
    } else {
      emiValue = principalNum * 
        monthlyRate * Math.pow(1 + monthlyRate, numberOfInstallments) / 
        (Math.pow(1 + monthlyRate, numberOfInstallments) - 1);
    }

    const totalPaymentValue = emiValue * numberOfInstallments;
    const totalInterestValue = totalPaymentValue - principalNum;
  ...
  const handleCopy = () => {
    if (emi !== null && totalInterest !== null && totalPayment !== null) {
      const result = `Monthly EMI: ₹${emi.toFixed(2)}\nTotal Interest: ₹${totalInterest.toFixed(2)}\nTotal Payment: ₹${totalPayment.toFixed(2)}`;
  ...
  const principalPercentage = totalPayment !== null && totalPayment > 0 
    ? ((parseFloat(principal) / totalPayment) * 100).toFixed(1) 
    : '0';
  const interestPercentage = totalPayment !== null && totalPayment > 0 
    ? ((totalInterest || 0) / totalPayment * 100).toFixed(1) 
    : '0';
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-muted/50 rounded-lg p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-foreground/60">Monthly EMI</p>
            <p className="text-lg font-semibold text-foreground">{formatCurrency(emi)}</p>
          </div>
          <div>
          ...
          <div className="col-span-2">
            <p className="text-foreground/60">Total Payment</p>
            <p className="text-lg font-semibold text-foreground">{formatCurrency(totalPayment)}</p>
          </div>
        </div>
        ...
        <div className="pt-3 border-t border-input">
          ...
          <div className="h-2 bg-input rounded-full overflow-hidden flex">
            <div 
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50 sticky top-0">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right font-medium text-foreground">EMI</th>
