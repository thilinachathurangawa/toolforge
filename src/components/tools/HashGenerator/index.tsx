...
import { Copy, Check, Hash as HashIcon } from 'lucide-react';
...
// Simple MD5 implementation (since Web Crypto API doesn't support MD5)
...
async function generateShaHash(message: string, algorithm: 'SHA-1' | 'SHA-256' | 'SHA-512'): Promise<string> {
...
export function HashGenerator() {
  ...
  const generateHashes = useCallback(async () => {
    ...
    for (const algo of selectedAlgorithms) {
      ...
      if (algo === 'MD5') {
        ...
        hash = await generateShaHash(inputText, algo as 'SHA-1' | 'SHA-256' | 'SHA-512');
      }
      hashResults.push({ algorithm: algo as HashResult['algorithm'], hash });
    }
  ...
  const handleCopy = (hash: string, algorithm: string) => {
    const hashToCopy = isUppercase ? hash.toUpperCase() : hash;
  ...
  }, [inputText, selectedAlgorithms, autoGenerate, generateHashes]);
    ...
    <div className="w-full space-y-6">
      <div className="p-6 border border-border rounded-xl bg-card space-y-6">
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            className="w-full min-h-[120px] p-3 border border-border rounded-lg bg-muted/50 text-foreground placeholder:text-muted-foreground resize-y focus:outline-none focus:ring-2 focus:ring-accent/50"
          />
        </div>

        {/* Algorithm Selection */}
        <div className="space-y-3">
          <label className="text-sm font-medium text-foreground">Select Algorithms</label>
      ...
      {results.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {results.map((result) => (
              <div key={result.algorithm} className="p-4 border border-border rounded-lg bg-muted/50 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-foreground">{result.algorithm}</span>
                  <button
                  ...
                  >
                    {copied === result.algorithm ? <Check size={16} /> : <Copy size={16} />}
